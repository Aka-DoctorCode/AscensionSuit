...
---@param parent frame The frame that owns this menu
---@param options table? Array of {text, func} for custom options
---@param config table? Optional position config {locked, point, x, y} to add Lock/Reset options
...
---Internal helper to create a basic menu frame.
function UX:createContextMenu(parent, options)
    ...
    menu:SetBackdropColor(unpack(styles.colors.backgroundDark))
    menu:SetBackdropBorderColor(unpack(styles.colors.surfaceHighlight))
    ...
    for i, opt in ipairs(options) do
        if opt.text == "---" then
            ...
            line:SetColorTexture(unpack(styles.colors.surfaceHighlight))
            ...
            btn:SetScript("OnEnter", function(self)
                ...
                self.bg:SetColorTexture(unpack(styles.colors.surfaceHighlight))
                self.bg:Show()
            end)
